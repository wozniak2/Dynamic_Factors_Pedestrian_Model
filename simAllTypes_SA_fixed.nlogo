...
;
; SENSITIVITY-ANALYSIS VARIANT — FIXED COPY
; This is a corrected copy of simAllTypes_SA.nlogo (the original is left
; untouched). Fixes applied here (search "FIX:" to find each one):
;   1. crowd-intensity is now rounded before being used as a list index in
;      setup-data, so non-integer values sampled by SA/sensitivity_fixed.py's
;      Saltelli sampling no longer crash `item`/`sprout-crowds`.
;   2. dijkstra-utility's dead uniform-range good-value/bad-value computation
;      (silently overwritten by a second, normal-distribution computation) has
;      been removed, keeping only the normal-distribution draw — this matches
;      Eq. 4-5 of the paper (φ̃_sen ~ N(φ_sen, σ=v)) and changes nothing
;      behaviourally, since the removed lines never took effect anyway.
;   3. The dispersion "visited" threshold in `go` now matches the paper's Eq. 8
;      and simAllTypes_clean.nlogo (`integration >= 0.05 * num-agen`), instead
;      of the unrelated constant `integration > 1`.
;   4. dijkstra-utility's "infinity" sentinel raised from 10^4 to 10^200 (matching
;      the plain `dijkstra` reporter) so it can't be mistaken for a real,
;      reachable cost on long accumulated paths.
;   5. Noise-patch selection and crowd-agent placement get the same crash
;      guards as in simAllTypes_clean_fixed.nlogo.
; NOT fixed here — this must be paired with SA/sensitivity_fixed.py, which
; corrects the parameter-name mismatch between the Python driver and this
; model's actual global names (attractor-strength, repeller-strength,
; noise-intensity, trip-distance, GIS-distance, segment-weight).
...
output-print "constructions done"
 ...
 ; FIX: clamp the requested count to the number of available patches so
 ...
 ask n-of noise-count noise-patches [
  set tag_noise "noise"
    ]
 ; ]
...
to setup-data
  ...
  ask pois-nodes [set color 44 set size 4 set shape "house" set hidden? false
    ...
    ; FIX: round crowd-intensity before using it as a list index — it is a
    ; continuous real when sampled by SA/sensitivity_fixed.py's Saltelli method,
    ; but `item`/`sprout-crowds` require a whole number.
    if item (round crowd-intensity) pois-intensity > crowd-tolerance [set tag_int "crowd" ]
    sprout-crowds item (round crowd-intensity) pois-intensity
    ask crowds [set shape "person" set size 2 set color grey
    ; FIX: fall back to the globally nearest node instead of crashing with
    ; "move-to nobody" if no node happens to fall within 25 patches.
  ...
  if (walkable-environment = TRUE) [
    if count walkers-here > 0 [
      ...
      set integration (integration + agent-num)
        ; FIX: was `integration > 1`, a constant unrelated to agent population
        ; size. This now matches the paper's Eq. 8 and simAllTypes_clean.nlogo:
        ; a patch counts as "visited" once traversal reaches 5% of all agents.
...
to-report dijkstra-utility [ start-node finish-node ] ;; Dijkstra utility
  ...
  ask nodes [
    set dijkstra-visited? false
    ; FIX: raised from 10^4 to 10^200 (matching the plain `dijkstra` reporter)
    ; so that an unvisited node's placeholder "infinity" can never be mistaken
    ; for a genuinely cheaper cost once dijkstra-distance has accumulated over
    ; a long path.
    set dijkstra-distance ifelse-value (self = start-node) [0] [10 ^ 200] ; 'infinity'
    set dijkstra-previous nobody
  ]
    ...
    ask current-node [ ifelse count my-routes < 2 [    ; ifelse calculates TAGS' costs only if my-links > 2
      ...
      ask route-neighbors with [not dijkstra-visited? ] [
       ...
       ;  ifelse count my-routes > 1 [
          ;; each turtle differ in-term of costs
       ;  ifelse count my-routes > 1 [
          ; FIX: this used to compute good-value/bad-value twice — once via a
          ; uniform draw over [lower,upper] (using now-removed lower-add/
          ; upper-add/lower-sub/upper-sub locals), then immediately overwritten
          ; by the normal-distribution draw below. Only the second computation
          ; ever took effect, and it is the one that matches the paper's Eq. 4-5
          ; (phi-tilde_sen ~ N(phi_sen, sigma=route-variability)), so the dead
          ; first computation has been removed. This changes nothing behaviourally.
