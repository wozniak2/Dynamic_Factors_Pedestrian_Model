...
FIXED COPY of SA/sensitivity.py (the original is left untouched).

FIX: `problem["names"]` previously used the paper's parameter labels
(attractor-sensitivity, repeller-sensitivity, noise-propagation, EOD,
node-stimulus-distance, segment-cost-weight) directly as NetLogo `set` targets
via `netlogo.command("set {0} {1}")`. Those names do not exist as NetLogo
globals in simAllTypes_SA.nlogo/simAllTypes_SA_fixed.nlogo — the sliders there
are named attractor-strength, repeller-strength, noise-intensity, trip-distance,
GIS-distance and segment-weight. Worse, "attractor-sensitivity" and
"distractor-sensitivity" are walkers-own (turtle) variables in that model, not
globals at all, so no name-only fix on the NetLogo side could make them
settable from the observer context. As originally committed, `simulation()`
below would raise a NetLogo runtime error on the very first sampled row for
6 of the 8 parameters. The names (and the EOD/trip-distance bounds, converted
using the model's 5.5 m patch size) are corrected below.

This script must be paired with simAllTypes_SA_fixed.nlogo, not the original
simAllTypes_SA.nlogo, since it also fixes the fractional-crowd-intensity-as-
list-index bug that this sampling approach would otherwise trigger.
"""
...
problem = {
    "num_vars": 8,
    "names": [
        "attractor-strength",       # was "attractor-sensitivity" (no such NetLogo global)
        "repeller-strength",        # was "repeller-sensitivity" (no such NetLogo global)
         "crowd-intensity",         # unchanged — already matched the NetLogo global
         "noise-intensity",         # was "noise-propagation" (no such NetLogo global)
         "trip-distance",           # was "EOD" (no such NetLogo global)
         "GIS-distance",            # was "node-stimulus-distance" (no such NetLogo global)
         "route-variability",       # unchanged — already matched the NetLogo global
         "segment-weight",          # was "segment-cost-weight" (no such NetLogo global)
    ],
    ...
    "bounds": [
        ...
        [0, 12],
        [0, 42],
        [40, 200],   # was [220, 1100] metres for "EOD"; trip-distance is in
                     # patches (1 patch = 5.5 m, see the model's distance-m
                     # reporter), so 220-1100 m -> ~40-200 patches
        [1, 20],
        [0, 0.7],
        [0.1, 1],    # was [0, 1]; matches the paper's Table 2 segment's cost
                     # weight range (0.1-1.0) for this parameter
    ],
...
# Load the Model
# FIX: point at the fixed NetLogo model (relative to `cwd`, pushed above, which
# is the repo root) — this script's parameter names/bounds only match
# simAllTypes_SA_fixed.nlogo, not the original simAllTypes_SA.nlogo.
netlogo.load_model('simAllTypes_SA_fixed.nlogo')
